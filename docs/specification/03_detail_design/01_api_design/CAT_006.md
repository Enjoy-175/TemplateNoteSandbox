...
- Step 4: 所有者チェック
  - ids に含まれるカテゴリを取得
  - 他ユーザー所有の id が存在する場合は E-403-CATEGORY-FORBIDDEN
  - details に失敗 id 一覧を付与

- Step 5: 一括削除
  - categories を user_id + ids で削除
  - トランザクション開始
  - 失敗: ロールバックし E-500-DB
...
- 例:
  {
    ...
    "details": {
      "results": [
        { "id": 5, "status": "failed", "reasonCode": "E-403-CATEGORY-FORBIDDEN", "message": "他のユーザーのカテゴリは操作できません。" }
      ]
    },
    "operation": "delete",
    "categoryId": null
...
- E-403-CATEGORY-FORBIDDEN
  - 発生条件: ids に他ユーザーのカテゴリが含まれる
  - Input例: ids = [5]（他ユーザー所有）
  - 検出Step: Step 4
